    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="办理信息" prop="banlixinxi">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.banlixinxi" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.banlixinxi" label="办理信息" prop="banlixinxi">
                    <span v-html="ruleForm.banlixinxi"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.fengmian = true;
          ...
          if(o=='banlixinxi'){
            this.ruleForm.banlixinxi = obj[o];
	    this.ro.banlixinxi = true;
            continue;
          }
          if(o=='yonghuzhanghao'){
	...
	this.ruleForm.banlixinxi = this.ruleForm.banlixinxi.replace(reg,'../../../ssm8ag2r/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `banshixinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.banshixinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
